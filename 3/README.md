...
本实验通过 socket 编程，利用多进程、多线程技术，实现一个简易的支持高性能并发 HTTP 服务器，并利用线程复用， I/O 复用，缓存，预测等机制，提高 HTTP 服务器的性能。
...
第一行是 HTTP 请求行，`GET` 代表 HTTP 方法，向指定的资源发出“显示”请求；`/hello.html` 为请求的资源路径；`HTTP/1.1` 为请求的协议版本。

接下来的行是请求行，`Host: 127.0.0.1:8000` 指明了请求的 Host，服务器的域名(用于虚拟主机 )，以及服务器所监听的传输控制协议端口号，这一字段是 HTTP 1.1 规定的必须的字段。
...
这两个接口使用时很容易出错，具体用法请同学们自行查阅手册，掌握各参数及返回值的意义。
...
实现并发有很多方案，如：`fork()` 创建新进程来处理请求，也可以用 `pthread` 来创建新线程，在此基础上，还可以创建线程池 `thread pool`，将请求分配给给预启动的线程进行处理。
...
真正的 HTTP Server 会尝试返回用户请求的资源内容（文件内容），请实现这一部分内容。
